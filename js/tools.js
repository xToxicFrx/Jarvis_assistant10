...
const TOOL_SCHEMAS = [
  ...
  { type: "function", function: { name: "add_homework", description: "Hausaufgabe fuer ein Fach anlegen. due_mode 'next_lesson' setzt die Faelligkeit automatisch auf die naechste Stunde des Fachs.", parameters: { type: "object", properties: { subject: { type: "string" }, title: { type: "string" }, due: { type: "string", description: "YYYY-MM-DD" }, due_mode: { type: "string", enum: ["date", "next_lesson"] }, priority: { type: "string", enum: ["low", "med", "high"] } }, required: ["subject", "title"] } } },
  ...
  { type: "function", function: { name: "add_subtask", description: "Unteraufgabe/Checklistenpunkt zu einer bestehenden Aufgabe hinzufuegen.", parameters: { type: "object", properties: { task: { type: "string", description: "Aufgabe (Stichwort oder id)." }, title: { type: "string" } }, required: ["task", "title"] } } },
...
async function runTool(name, args, ctx) {
  switch (name) {
    ...
    case "add_homework": { const t = Store.addTask({ title: args.title, type: "homework", subject: args.subject, due: args.due, dueMode: args.due_mode === "next_lesson" ? "nextLesson" : "date", priority: args.priority }); return `Hausaufgabe: ${t.subject || "?"} - "${t.title}"${t.due ? ` (bis ${t.due})` : t.dueMode === "nextLesson" ? " (kein Stundenplan-Eintrag — bitte Datum nachtragen)" : ""}.`; }
    ...
    case "add_subtask": { const t = Store.findTask(args.task); if (!t) return "Keine passende Aufgabe gefunden."; Store.updateTask(t.id, { subtasks: (t.subtasks || []).concat([{ title: args.title }]) }); return `Unteraufgabe zu "${t.title}" hinzugefuegt: "${args.title}".`; }
